...
class CommandPlay extends SlashCommand {
    ...
    async run (ctx) {
        ...
        if (pornPattern(ctx.options.track)) return this.creator.ui.say(ctx, 'no', "The URL you're requesting to play is not allowed.");
        ...
        if (!currentVc) {
            ...
            if (!permissions) return this.creator.ui.send(ctx, 'MISSING_CONNECT', vc.id);
            ...
            if (vc.type === 'stage') {
                ...
                if (!stageMod) {
                    ...
                    if (!requestToSpeak) {
                        ...
                        return this.creator.ui.send(ctx, 'MISSING_SPEAK', vc.id);
            ...
            if (queue) {
                ...
                if (maxQueueLimit) {
                    ...
                    if (queueMemberSize >= maxQueueLimit) {
                        return this.creator.ui.say(ctx, 'no', `You are only allowed to add a max of ${maxQueueLimit} entr${maxQueueLimit === 1 ? 'y' : 'ies'} to the queue.`);
                    }
                }
            }
        }
        ...
        try {
            ...
            return this.creator.ui.custom(ctx, process.env.EMOJI_MUSIC, process.env.COLOR_MUSIC, `Requested \`${ctx.options.track}\``);
        } catch (err) {
            this.creator.logger.error(err.stack); // Just in case.
            return this.creator.ui.say(ctx, 'error', `An unknown error occured:\n\`\`\`js\n${err.name}: ${err.message}\`\`\``, 'Player Error');
        }
    }
}
