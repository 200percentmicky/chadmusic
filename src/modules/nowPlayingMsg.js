...
async function nowPlayingMsg (queue, song) {
    ...
    await channel.client.utils.setVcStatus(vc, `${process.env.EMOJI_MUSIC} ${song.name} [${song.formattedDuration}] (${song.user.displayName} - ${song.user.username})`.substring(0, 496) + '...');
}
