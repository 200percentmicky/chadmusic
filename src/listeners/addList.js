  ...
  async exec (queue, playlist) {
    ...
    if (!dj) {
      ...
      if (maxQueueLimit) {
        ...
        return this.client.ui.say(channel, 'ok', `${playlist.user} added the playlist **${playlist.name}** with **${playlist.songs.length}** entr${playlist.songs.length === 1 ? 'y' : 'ies'}. However, only the first ${maxQueueLimit > 1 ? `${maxQueueLimit} entries` : 'entry'} was added due to queue limits on this server.`);
      }
    }
