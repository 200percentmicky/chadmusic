...
module.exports = class CommandSummon extends Command {
  ...
  async exec (message) {
    ...
    if (currentVc) {
      if (vc.id !== currentVc.id) return message.error('I\'m currently binded to a different voice channel.')
      else return message.warn('I\'m already in a voice channel. Let\'s get this party started!')
    } else {
      vc.join()
      message.react('📥')
