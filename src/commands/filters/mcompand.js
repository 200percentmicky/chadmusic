...
module.exports = class CommandMcompand extends Command
{
    constructor()
    {
        super('mcompand', {
            ...
            category: '🗣 Filter',
            description: {
                text: 'Applies the mcompand filter to the player.',
                filter: 'mcompand'
            },
            channel: 'guild',
    ...
    async exec(message)
    {
        ...
        if (currentVc)
        {
            await this.client.player.setFilter(message.guild.id, 'mcompand');
            return message.ok(`Applied filter: **mcompand**`);
        } else {
