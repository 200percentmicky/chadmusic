...
module.exports = class CommandFlanger extends Command
{
    constructor()
    {
        super('flanger', {
            ...
            category: '🗣 Filter',
            description: {
                text: 'Adds a flanger filter to the player.',
                filter: 'flanger'
            },
            channel: 'guild',
    ...
    async exec(message)
    {
        ...
        if (currentVc)
        {
            await this.client.player.setFilter(message.guild.id, 'flanger');
            return message.ok(`Applied filter: **Flanger**`);
        } else {
