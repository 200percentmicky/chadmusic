...
module.exports = class CommandHaas extends Command {
  constructor () {
    super('haas', {
      aliases: ['haas'],
      category: '🗣 Filter',
      description: {
        text: 'Adds the haas filter to the player.',
        filter: 'haas'
      },
      channel: 'guild',
  ...
  async exec (message) {
    ...
    if (currentVc) {
      await this.client.player.setFilter(message.guild.id, 'haas')
      return message.ok('Applied filter: **haas**')
    } else {
