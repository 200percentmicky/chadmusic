...
module.exports = class CommandPhaser extends Command {
  constructor () {
    super('phaser', {
      aliases: ['phaser'],
      category: '🗣 Filter',
      description: {
        text: 'Applies phaser to the player.',
        filter: 'aphaser'
      },
      channel: 'guild',
  ...
  async exec (message) {
    ...
    if (currentVc) {
      await this.client.player.setFilter(message.guild.id, 'phaser')
      return message.ok('Applied filter: **Phaser**')
    } else {
