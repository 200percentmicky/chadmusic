...
module.exports = class CommandGate extends Command
{
    constructor()
    {
        super('gate', {
            ...
            category: '🗣 Filter',
            description: {
                text: 'Adds a gate filter to the player.',
                filter: 'agate'
            },
            channel: 'guild',
    ...
    async exec(message)
    {
        ...
        if (currentVc)
        {
            await this.client.player.setFilter(message.guild.id, 'gate');
            return message.ok(`Applied filter: **Gate**`);
        } else {
