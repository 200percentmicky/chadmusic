    ...
    if (message.author.id !== tag.user_id) {
      if (message.channel.permissionsFor(message.author.id).has(['MANAGE_GUILD'])) {
        if (this.client.settings.get(message.guild.id, 'taglog')) {
          ...
          tagLog.send(`✂ **${message.author.username}**#${message.author.discriminator} (\`${message.author.id}\`) deleted the tag: \`${args[1]}\``) // May convert this into an embed later, but this will do.
        }
